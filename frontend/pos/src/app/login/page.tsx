  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-gradient-to-br from-[#0f172a] via-[#111827] to-slate-900 px-4">
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute -left-40 -top-40 h-96 w-96 animate-float rounded-full bg-violet-500/10 blur-3xl" />
        <div className="absolute -bottom-32 -right-32 h-80 w-80 animate-float rounded-full bg-violet-500/8 blur-3xl" style={{ animationDelay: "1.5s" }} />
      ...
      <div className="w-full max-w-md animate-fade-in">
        <div className="mb-8 text-center">
          <div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-3xl bg-gradient-to-br from-violet-500 to-violet-600 text-white text-2xl font-black shadow-xl shadow-violet-500/25 ring-1 ring-white/10">
            <Sparkles aria-hidden="true" className="h-7 w-7" />
          </div>
          <h1 className="text-3xl font-black tracking-tight text-white">
            INDEX <span className="bg-gradient-to-r from-violet-400 to-purple-400 bg-clip-text text-transparent font-light">POS</span>
          </h1>
          ...
          <form className="grid gap-5" onSubmit={handleSubmit}>
            <label className="grid gap-1.5" htmlFor="login-email">
              ...
              <input
                ...
                className="h-12 w-full rounded-xl border border-white/[0.08] bg-white/[0.04] px-4 text-sm font-semibold text-white placeholder-slate-500 shadow-inner shadow-black/5 transition-all duration-300 focus:border-violet-500/50 focus:bg-white/[0.08] focus:ring-2 focus:ring-violet-500/20 focus:shadow-glow outline-none"
            ...
            <button
              type="submit"
              disabled={loading}
              className="inline-flex h-12 items-center justify-center gap-2.5 rounded-xl bg-gradient-to-r from-violet-500 to-violet-600 px-5 text-sm font-bold text-white shadow-lg shadow-violet-500/20 transition-all duration-300 hover:from-violet-400 hover:to-violet-500 hover:shadow-violet-500/30 hover:shadow-glow hover:-translate-y-0.5 active:translate-y-0 disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:translate-y-0 disabled:shadow-none outline-none focus:ring-2 focus:ring-violet-500/40"
            >
              {loading ? (
