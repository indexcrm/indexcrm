    ...
    <div className="min-h-screen bg-[#f4f5f9] text-slate-900">
      <aside className="fixed inset-y-0 left-0 hidden w-64 border-r border-white/[0.04] bg-gradient-to-b from-[#0f172a] via-[#111827] to-[#0f172a] text-white lg:block shadow-2xl shadow-black/10 z-20">
        <div className="flex h-14 items-center gap-3 border-b border-white/[0.04] px-5">
          <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-gradient-to-br from-amber-500 to-amber-600 text-white text-sm font-black shadow-lg shadow-amber-500/20 ring-1 ring-white/10">
            I
          </div>
          <div>
            <div className="text-sm font-black tracking-tight">
              <span className="text-white">INDEX</span>{" "}
              <span className="text-amber-400 font-light">{roleLabel === "Egasi" ? "Ega" : roleLabel}</span>
            </div>
            ...
            return (
              <a
                ...
                className={`group flex h-10 items-center gap-3 rounded-xl px-3.5 text-sm font-semibold transition-all duration-200 ${
                  active
                    ? "bg-gradient-to-r from-amber-500/90 to-amber-600/90 text-white shadow-lg shadow-amber-500/15"
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-10 flex h-14 items-center justify-between border-b border-amber-100/50 bg-white/80 px-4 shadow-soft backdrop-blur-xl lg:px-6">
          <div className="min-w-0 flex items-center gap-3">
            <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-to-br from-amber-500 to-amber-600 text-white text-[10px] font-black shadow-sm lg:hidden">
              I
            </div>
            <div>
          ...
          <div className="flex shrink-0 items-center gap-2">
            <a
              href="/"
              className="inline-flex items-center gap-2 rounded-xl border border-amber-100/60 bg-white px-3.5 py-2 text-xs font-bold text-amber-600 shadow-sm transition-all duration-200 hover:bg-amber-50 hover:border-amber-200 hover:shadow-md active:scale-95"
            >
              POS ochish
            </a>
            <LogoutButton />
        ...
        <nav className="flex gap-1.5 overflow-x-auto border-b border-amber-100/50 bg-white/50 px-3 py-2 lg:hidden backdrop-blur-sm">
          {visibleNavItems.map((item) => {
            const Icon = item.icon;
            const active = isActive(pathname, item.href);
            return (
        ...
        <main className="pb-8">
          {hasAccess ? (
            ...
            <div className="p-4 lg:p-6">
              <section className="rounded-2xl border border-amber-100/50 bg-white shadow-soft overflow-hidden">
                <EmptyState
                  title="Bu bo'lim mavjud emas"
                  description="Joriy rolingiz bu boshqaruv sahifasiga kirish huquqiga ega emas."
                />
                <div className="flex justify-center border-t border-amber-100/50 p-4">
                  <a
                    href="/"
                    className="inline-flex items-center gap-2 rounded-xl border border-amber-100/60 bg-white px-4 py-2.5 text-sm font-bold text-amber-600 shadow-sm transition-all duration-200 hover:bg-amber-50 hover:shadow-md active:scale-95"
                  >
