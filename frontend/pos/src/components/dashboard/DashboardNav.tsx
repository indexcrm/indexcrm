  ...
  return (
    <div className="min-h-screen bg-[#f4f5f9] text-slate-900">
      <aside className="fixed inset-y-0 left-0 hidden w-64 border-r border-white/[0.04] bg-gradient-to-b from-[#0f172a] via-[#111827] to-[#0f172a] text-white lg:block shadow-2xl shadow-black/10 z-20">
        <div className="flex h-14 items-center gap-3 border-b border-white/[0.04] px-5">
          ...
          <div>
            <div className="text-sm font-black tracking-tight">
              ...
              <span className="text-red-400 font-light">{roleLabel === "Egasi" ? "Ega" : roleLabel}</span>
            </div>
            ...
            return (
              <a
                ...
                className={`group flex h-10 items-center gap-3 rounded-xl px-3.5 text-sm font-semibold transition-all duration-200 ${
                  active
                    ? "bg-gradient-to-r from-red-500/90 to-red-600/90 text-white shadow-lg shadow-red-500/15"
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-white/[0.04]">
          <div className="flex items-center gap-2 text-[11px] text-slate-500">
            <div className="h-1.5 w-1.5 rounded-full bg-red-400 shadow-sm shadow-red-400/30" />
            INDEX POS v0.1.0
          </div>
        </div>
      </aside>
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-10 flex h-14 items-center justify-between border-b border-red-100/50 bg-white/80 px-4 shadow-soft backdrop-blur-xl lg:px-6">
          <div className="min-w-0 flex items-center gap-3">
            <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-to-br from-red-500 to-red-600 text-white text-[10px] font-black shadow-sm lg:hidden">
              I
            </div>
            <div>
          ...
          <div className="flex shrink-0 items-center gap-2">
            <a
              href="/"
              className="inline-flex items-center gap-2 rounded-xl border border-red-100/60 bg-white px-3.5 py-2 text-xs font-bold text-red-600 shadow-sm transition-all duration-200 hover:bg-red-50 hover:border-red-200 hover:shadow-md active:scale-95"
            >
              POS ochish
            </a>
            <LogoutButton />
        ...
        <nav className="flex gap-1.5 overflow-x-auto border-b border-red-100/50 bg-white/50 px-3 py-2 lg:hidden backdrop-blur-sm">
          {visibleNavItems.map((item) => {
            const Icon = item.icon;
            const active = isActive(pathname, item.href);
            return (
        ...
        <main className="pb-8">
          {hasAccess ? (
            ...
            <div className="p-4 lg:p-6">
              <section className="rounded-2xl border border-red-100/50 bg-white shadow-soft overflow-hidden">
                <EmptyState
                  title="Bu bo'lim mavjud emas"
                  description="Joriy rolingiz bu boshqaruv sahifasiga kirish huquqiga ega emas."
                />
                <div className="flex justify-center border-t border-red-100/50 p-4">
                  <a
                    href="/"
                    className="inline-flex items-center gap-2 rounded-xl border border-red-100/60 bg-white px-4 py-2.5 text-sm font-bold text-red-600 shadow-sm transition-all duration-200 hover:bg-red-50 hover:shadow-md active:scale-95"
                  >
