      ...
      <header className="no-print flex h-14 shrink-0 items-center justify-between bg-gradient-to-r from-[#0f172a]/95 via-[#111827]/95 to-[#0f172a]/95 px-5 text-white border-b border-white/[0.04] backdrop-blur-lg">
        <div className="flex items-center gap-3">
          ...
          <div className="hidden sm:flex items-center gap-2">
            <span className="rounded-lg bg-blue-500/10 px-2 py-0.5 text-[9px] font-black tracking-widest text-blue-400 border border-blue-400/20 uppercase shadow-sm">
              Kassir
            </span>
          </div>
        </div>
        <div className="flex items-center gap-4 text-sm font-medium">
          <div className="hidden items-center gap-3 text-slate-400 lg:flex">
            {cashierName || cashierEmail ? (
              <div className="flex items-center gap-2">
                <div className="h-6 w-6 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center text-[10px] font-bold text-white shadow-sm">
            ...
            <div className="flex items-center gap-3 text-xs">
              <div className="flex items-center gap-1.5">
                <div className={`h-1.5 w-1.5 rounded-full ${branchId ? 'bg-blue-400 shadow-blue-400/40' : 'bg-slate-600'} ${branchId ? 'shadow-sm' : ''}`} />
                <span className="text-slate-400">Filial</span>
                <span className="text-slate-300 font-semibold">{branchId ? "Ha" : "Yo'q"}</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className={`h-1.5 w-1.5 rounded-full ${warehouseId ? 'bg-blue-400 shadow-blue-400/40' : 'bg-slate-600'} ${warehouseId ? 'shadow-sm' : ''}`} />
                <span className="text-slate-400">Ombor</span>
                <span className="text-slate-300 font-semibold">{warehouseId ? "Ha" : "Yo'q"}</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className={`h-1.5 w-1.5 rounded-full ${activeShiftId ? 'bg-blue-400 shadow-blue-400/40' : 'bg-slate-600'} ${activeShiftId ? 'shadow-sm' : ''}`} />
                <span className="text-slate-400">Smena</span>
                <span className={`font-semibold ${activeShiftId ? 'text-blue-400' : 'text-slate-300'}`}>{activeShiftId ? "Ochiq" : "Yo'q"}</span>
              </div>
            </div>
          </div>
