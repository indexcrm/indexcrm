        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xl font-black tracking-tight text-white">
              ...
              <span className="text-amber-400 font-light ml-1">POS</span>
            </span>
          </div>
          <div className="hidden sm:flex items-center gap-2">
            <span className="rounded-lg bg-amber-500/10 px-2 py-0.5 text-[9px] font-black tracking-widest text-amber-400 border border-amber-400/20 uppercase shadow-sm">
              Kassir
            </span>
          </div>
        </div>
        <div className="flex items-center gap-4 text-sm font-medium">
          <div className="hidden items-center gap-3 text-slate-400 lg:flex">
            {cashierName || cashierEmail ? (
              <div className="flex items-center gap-2">
                <div className="h-6 w-6 rounded-full bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center text-[10px] font-bold text-white shadow-sm">
            ...
            <div className="flex items-center gap-3 text-xs">
              <div className="flex items-center gap-1.5">
                <div className={`h-1.5 w-1.5 rounded-full ${branchId ? 'bg-emerald-400 shadow-emerald-400/40' : 'bg-slate-600'} ${branchId ? 'shadow-sm' : ''}`} />
                <span className="text-slate-400">Filial</span>
                <span className="text-slate-300 font-semibold">{branchId ? "Ha" : "Yo'q"}</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className={`h-1.5 w-1.5 rounded-full ${warehouseId ? 'bg-emerald-400 shadow-emerald-400/40' : 'bg-slate-600'} ${warehouseId ? 'shadow-sm' : ''}`} />
                <span className="text-slate-400">Ombor</span>
                <span className="text-slate-300 font-semibold">{warehouseId ? "Ha" : "Yo'q"}</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className={`h-1.5 w-1.5 rounded-full ${activeShiftId ? 'bg-emerald-400 shadow-emerald-400/40' : 'bg-slate-600'} ${activeShiftId ? 'shadow-sm' : ''}`} />
