    ...
    >
      ...
      >
        <div className={`flex h-5 w-5 items-center justify-center rounded-full ${
          ...
          sessionReady ? "bg-violet-200/60" : "bg-amber-200/60"
        }`}>
          {shiftLoading ? (
      ...
      <div className="mb-4 flex flex-col gap-1.5 rounded-xl bg-gradient-to-b from-slate-50 to-white p-3.5 text-sm shadow-sm border border-slate-200/70">
        ...
        <div className="flex justify-between items-center py-1 pt-2 mt-1 border-t border-slate-200/60">
          ...
          <span className={`font-bold px-2.5 py-0.5 rounded-full text-[10px] tracking-wide ${activeShiftId ? "bg-violet-100 text-violet-700 ring-1 ring-inset ring-violet-200" : "bg-rose-50 text-rose-600 ring-1 ring-inset ring-rose-200"}`}>{activeShiftId ? "Ochiq" : "Yo'q"}</span>
        </div>
      </div>
