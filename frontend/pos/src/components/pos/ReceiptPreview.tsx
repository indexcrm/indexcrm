    ...
    <section className="print-panel flex min-h-0 flex-1 flex-col bg-[#131b2e] p-4">
      <div className="no-print mb-2 flex items-center justify-between gap-2">
        ...
        <div className="flex gap-2">
          <IconButton
            icon={<ShoppingCart aria-hidden="true" className="h-3.5 w-3.5" />}
            label="Yangi savdo"
            onClick={onNewSale}
            disabled={!sale}
          ...
          />
          <IconButton
            icon={<Printer aria-hidden="true" className="h-3.5 w-3.5" />}
            label="Chop etish"
      ...
      <div className="receipt-paper min-h-0 flex-1 overflow-y-auto rounded-xl border border-slate-700/30 bg-[#131b2e] p-4 font-mono text-[13px] leading-tight shadow-sm">
        {!sale ? (
          ...
          <div className="flex h-full items-center justify-center gap-2 text-center font-sans text-sm font-bold text-slate-600">
          ...
          <>
            {receiptQuery.isError ? (
              <div className="no-print mb-3 rounded border border-amber-300 bg-amber-50 px-3 py-2 font-sans text-xs font-bold text-amber-900">
            ...
            <div className="text-center">
              <div className="text-[15px] font-black uppercase">{branchName}</div>
              <div>{pickText(receipt?.branch, ["address"], "")}</div>
              <div>{pickText(receipt?.branch, ["phone"], "")}</div>
              <div className="mt-2 font-black">Chek #{receiptNumber}</div>
              <div>Sana: {formatDate(saleDate)}</div>
              <div>Kassir: {cashierName}</div>
              {sessionLabel ? <div>Sessiya: {sessionLabel}</div> : null}
              {customerName ? <div>Mijoz: {customerName}</div> : null}
            </div>
            ...
            {receiptItems.map((item, index) => {
              ...
              return (
                <div key={String(row.id ?? row.product_id ?? index)} className="mb-2">
                  ...
                  {sku ? <div className="text-[11px] uppercase">SKU: {sku}</div> : null}
