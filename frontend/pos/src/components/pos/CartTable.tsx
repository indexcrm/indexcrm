...
export function CartTable({ items, onQuantityChange, onRemove }: CartTableProps) {
  return (
    <section className="flex min-h-0 flex-1 flex-col bg-white">
      <div className="grid grid-cols-[1fr_130px_110px_48px] border-b border-slate-200/80 bg-slate-50/80 px-5 py-2.5 text-[10px] font-bold uppercase tracking-widest text-slate-400">
        <span>Mahsulot</span>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto divide-y divide-slate-100/80">
        {items.length === 0 ? (
          <div className="flex h-full items-center justify-center p-6 text-center">
            <div className="rounded-2xl border-2 border-dashed border-slate-200 bg-white p-12 max-w-sm animate-fade-in">
              ...
              <div className="mt-2 flex items-center justify-center gap-2 text-sm font-semibold text-slate-400">
          ...
          items.map((item) => (
            <div
              key={item.product.id}
              className="group grid min-h-[5.5rem] grid-cols-[1fr_130px_110px_48px] items-center gap-3 bg-white px-5 py-3.5 transition-all duration-150 hover:bg-blue-50/20 hover:shadow-sm"
            >
              <div className="min-w-0">
                <div className="truncate text-[15px] font-bold text-slate-900 tracking-tight group-hover:text-blue-700 transition-colors">
                  {item.product.name}
                </div>
                <div className="flex items-center gap-1.5 text-xs font-semibold text-slate-400 mt-0.5">
                  <span className="text-slate-500">{item.product.sku || item.product.barcode || "-"}</span>
                  <span className="text-slate-300">&bull;</span>
                  <span className="text-slate-500">{formatMoney(item.price)}</span>
                </div>
              </div>
              <div className="flex items-center justify-center gap-0.5 rounded-lg border border-slate-200 bg-white shadow-sm">
                <IconButton
                  ...
                  hideLabel
                  className="h-8 w-8 rounded-l-lg rounded-r-none bg-transparent text-slate-500 hover:bg-slate-100 border-0 shadow-none"
                />
                <input
                  value={item.quantity}
                  onChange={(event) =>
                  ...
                  className="h-8 w-10 border-0 bg-transparent text-center text-sm font-bold text-slate-900 focus:ring-0"
                  inputMode="decimal"
                  min="0"
                  step="1"
                />
              ...
              <div className="text-right text-base font-black text-slate-900 tracking-tight">
